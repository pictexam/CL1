...
/* function print_inorder() will print inorder of nodes. Here we are also printing label of each node oftree(DAG) */
...
int main(){
    ...
    int i,temp;
    dag d;
    cout << "\nEnter root of tree:"; /* Inserting nodes into tree(DAG) */
    cin >> val;
    d.insert(&root,val);
    ...
    d.initializestack(root); /* Initializing Stack contents and top variable */
    cout << "\nInorder Display:\n";  /* Printing inorder of nodes of tree(DAG) */
    d.print_inorder(root);
    cout << "\nAssembly Code:\n";  /* Printing assembly code w.r.t. labels of tree(DAG) */
    d.gencode(root);
    d.deltree(root);   /* Deleting all nodes of tree */
    return 0;
}